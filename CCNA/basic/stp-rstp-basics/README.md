...
### 2. IEEE 802.1w Rapid  BPDU Header
...
* - **Hello Time:** 2 seconds (the interval at which Configuration BPDUs are generated by the Root Bridge and propagated through the Spanning Tree topology).
* - **Forward Delay:** 15 seconds (the time spent in both the Listening and Learning states before reaching Forwarding).
* - **Max Age:** 20 seconds (the maximum duration a switch retains received BPDU information before considering it expired).

#### Operational Rules and Scope
Configured on the Root Bridge: Spanning Tree timers should be configured on the active Root Bridge for the VLAN. The Root Bridge advertises the timer values in its BPDUs, and downstream switches use those operational values for the spanning-tree instance.

Network Diameter Impact: Manually changing individual timer parameters can lead to suboptimal convergence or unexpected topology behavior if the values are not appropriate for the network diameter. Cisco recommends using the diameter option when adjusting timers for larger topologies rather than manually tuning individual values.
...
If specific custom timer values are required, they can be configured directly on the designated Root Bridge:
...
**NOTE:** These values are used for demonstration purposes only. In production environments, timer values should be changed only when the topology and convergence requirements justify the change.


### Verification Commands
To confirm that the custom timers are active on the Root Bridge:
...
On downstream non-root switches, the Root ID section reflects the operational timer values advertised by the Root Bridge, while the Bridge ID section shows the locally configured timer values:
